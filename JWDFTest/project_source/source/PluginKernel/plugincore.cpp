...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(0, "Bypass", "OFF, ON", "OFF");
...
bool PluginCore::reset(ResetInfo& resetInfo)
{
	...
	hpfl.reset(resetInfo.sampleRate);
	hpfr.reset(resetInfo.sampleRate);
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFMono)
    {
		...
		double dryChannelData = channelData;

		channelData = hpfl.processAudioSample(channelData);

		channelData = (channelData * BranchlessBypass[bypass]) + (dryChannelData * bypass);
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		...
		double channelDataL = processFrameInfo.audioInputFrame[0];
		double channelDataR = processFrameInfo.audioInputFrame[0];

		double dryChannelDataL = channelDataL;
		double dryChannelDataR = channelDataR;

		channelDataL = hpfl.processAudioSample(channelDataL);
		channelDataR = hpfr.processAudioSample(channelDataR);

		channelDataL = (channelDataL * BranchlessBypass[bypass]) + (dryChannelDataL * bypass);
		channelDataR = (channelDataR * BranchlessBypass[bypass]) + (dryChannelDataR * bypass);

		processFrameInfo.audioOutputFrame[0] = channelDataL;
		processFrameInfo.audioOutputFrame[1] = channelDataR;
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		// --- pass through code: change this with your signal processing
		double channelDataL = processFrameInfo.audioInputFrame[0];
		double channelDataR = processFrameInfo.audioInputFrame[1];
